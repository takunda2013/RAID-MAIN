...
def make_demo_pdf(result: DemoResult, source_text: str) -> bytes:
    lines = [
        "AI Essay Detector Demo Report",
        ...
        dedent(
            ...
            <div class="app-title">
                <div class="hero-panel">
                    ...
                    <h1>Preserve the Purity of Your Writing</h1>
                    <div class="subtitle">
...
def render_input() -> tuple[str, str]:
    ...
    with st.container(border=False):
        ...
        def _on_file_change() -> None:
            ...
            if uploaded is None:
                return
            extracted_text = extract_uploaded_text(uploaded)
            if not extracted_text:
                st.session_state["file_extract_failed"] = True
                return
            st.session_state["essay_text"] = extracted_text
            st.session_state["last_uploaded_file"] = getattr(uploaded, "name", None)
            st.session_state["result"] = None
            st.session_state["file_extract_failed"] = False
        ...
        if uploaded_file is not None and st.session_state.get("file_extract_failed"):
            st.warning(
                "File received. Text extraction is not available for this file in the current environment, "
                "so the static demo result will still be used."
            )
        ...
        st.markdown(f'<div class="small-note">Word Count: {words} words</div>', unsafe_allow_html=True)
        ...
        st.markdown(
            f'<div class="small-note">* Minimum {MIN_WORDS} words recommended.</div>',
            unsafe_allow_html=True,
        )
...
def render_results(result: DemoResult | None) -> None:
            ...
            <div class="box">
                ...
                <div class="stat-grid">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-caption">Human, AI, paraphrased AI</div>
                    </div>
                </div>
            </div>
            """,
    ...
    st.markdown(
        ...
        <div class="result-head">
            ...
            <div class="confidence">{result.confidence}%<span>High confidence</span></div>
        </div>
        ...
        <div class="stat-grid">
            <div class="stat-card">
                ...
                <div class="stat-caption">Demo classifier score</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Decision strength</div>
                <div class="stat-value">{result.confidence}%</div>
                <div class="stat-caption">Confidence band: high</div>
            </div>
            <div class="stat-card">
    ...
    st.markdown(
        f'<div class="small-note">Demo AI probability: {result.ai_probability}%</div>',
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
